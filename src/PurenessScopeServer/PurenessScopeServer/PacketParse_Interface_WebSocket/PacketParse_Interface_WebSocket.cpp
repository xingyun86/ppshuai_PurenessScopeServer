...
extern "C"
{
    ...
    uint8 ReadDataPacketInfo(const char* pData, uint32& u4DataLen, uint16& u2CommandID, uint32& u4PacketLen, _Packet_Info* pPacketInfo)
    {
        ...
        //寻找第一个PKG_MSG_CODE,用于界定数据包信息的长度
		pCommand = (char* )ACE_OS::strstr(pData, PKG_MSG_CODE);

        if(NULL == pCommand)
        {
            //如果找不到PKG_MSG_CODE，说明数据包异常，断开这个链接
            return (uint8)PACKET_GET_ERROR;
        }

        if((int)(pCommand - pData) == 0)
        {
            //找不到前面的数据头，返回失败
            return (uint8)PACKET_GET_ERROR;
        }
        ...
        char* pDataLen = NULL;
		pDataLen = (char* )ACE_OS::strstr(pData, PKG_MSG_DATA);

        if(NULL == pDataLen)
        {
            //如果找不到PKG_MSG_DATA，说明这个命令包是错误的，断开连接
            return (uint8)PACKET_GET_ERROR;
        }
        else
        {
			int nCommandID = 0;
            if(pDataLen - pCommand == 0)
            {
                //说明没有找到数据头，返回失败
                return (uint8)PACKET_GET_ERROR;
            }
			...
			memcpy_safe((char*)pData, (uint32)(pCommand - pData), szTemp, (uint32)MAX_BUFF_100);

			//十六进制字符串转成整形
			sscanf(szTemp, "%x", &nCommandID);

			ACE_OS::memset(szTemp, 0, sizeof(szTemp));
            memcpy_safe((char* )(pCommand + 1), (uint32)(pDataLen - pCommand) - 1, szTemp, (uint32)MAX_BUFF_100);

			pPacketInfo->m_u2PacketCommandID = (uint16)nCommandID;
            u4PacketLen = (uint32)ACE_OS::atoi(szTemp);
    ...
    uint8 WebSocketDisposeDataIn(_WebSocketInfo* pWebSocketInfo, ACE_Message_Block* pCurrMessage, IMessageBlockManager* pMessageBlockManager, _Packet_Info* pPacketInfo)
    {
        ...
        if(u1Ret == PACKET_GET_ENOUGTH)
        {
            ...
            //去掉包头，只留数据体在包体内
            char* pInfo = (char* )ACE_OS::strstr(pWebSocketInfo->m_szDecryptData, PKG_MSG_DATA);
