...
int LoadModuleData(CServerObject* pServerObject)
{
    ...
    PControlInfo pControlInfo = pServerObject->GetControlListen()->CreateListenSnapshot(nControlInfoNum);
	if (pControlInfo)
	{
		for (int i = 0; i < nControlInfoNum; i++)
		{
			OUR_DEBUG((LM_INFO, "[Base LoadModuleData] Listen IP=%s, Pord=%d.\n", pControlInfo[i].m_szListenIP, pControlInfo[i].m_u4Port));
		}
		pServerObject->GetControlListen()->ReleaseListenSnapshot(&pControlInfo);
	}
