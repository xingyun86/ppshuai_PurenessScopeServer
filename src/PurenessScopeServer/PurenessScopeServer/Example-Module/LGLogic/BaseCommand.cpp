...
void CBaseCommand::Init(const char* pFileName)
{
	...
	int nControlnfoNum = 0;
	PControlInfo pControlInfo = m_pServerObject->GetControlListen()->CreateListenSnapshot(nControlnfoNum);
	if (pControlInfo)
	{
		if (nControlnfoNum > 0)
		{
			//自动得到当前开放的IP和端口，然后记录下来，发送给远端LS
			m_objLSServer.Set_LG_Info(pControlInfo[0].m_szListenIP,
				pControlInfo[0].m_u4Port,
				(uint32)m_pServerObject->GetControlListen()->GetServerID());
		}
		m_pServerObject->GetControlListen()->ReleaseListenSnapshot(&pControlInfo);
	}
