...
{
	//处理接收到的握手数据，返回握手命令
	char szResponseData[MAX_BUFF_500] = {'\0'};
	...
	//设置返回字符串
	sprintf_safe(szResponseData, MAX_BUFF_500, HANDIN_RETURN_DATA, strServerKey.c_str());

	OUR_DEBUG((LM_INFO, "[CBaseCommand::DoMessage_HandIn]ServerKey=%s.\n", strServerKey.c_str()));
	
	//IBuffPacket* pResponsesPacket = m_pServerObject->GetPacketManager()->Create();
	////数据原样奉还
	//pResponsesPacket->WriteStream(szResponseData, (uint32)ACE_OS::strlen(szResponseData));
	...
	{
		//发送全部数据
		const char * pszResponseData = szResponseData;
		m_pServerObject->GetConnectManager()->PostMessage(pMessage->GetMessageBase()->m_u4ConnectID, pszResponseData,
			(uint32)ACE_OS::strlen(pszResponseData), SENDMESSAGE_JAMPNOMAL, u2PostCommandID, PACKET_SEND_IMMEDIATLY, PACKET_IS_SELF_RECYC);
		////发送全部数据
		//m_pServerObject->GetConnectManager()->PostMessage(pMessage->GetMessageBase()->m_u4ConnectID, pResponsesPacket,
		//	SENDMESSAGE_JAMPNOMAL, u2PostCommandID, PACKET_SEND_IMMEDIATLY, PACKET_IS_SELF_RECYC);
	}
	else
	{
...
int CBaseCommand::DoMessage_DataIn(IMessage* pMessage, bool& bDeleteFlag)
{
	...
	strDataIn.append(BodyPacket.m_pData, BodyPacket.m_nDataLen);

	//打印接收后的数据体
	OUR_DEBUG((LM_INFO, "[CBaseCommand::DoMessage_DataIn](%d)%s\n", bDeleteFlag, strDataIn.c_str()));

	//数据原样返回
	char szResponseData[MAX_BUFF_1024] = {'\0'};
	uint32 u4ResponseDataLen = MAX_BUFF_1024;
	...
	int nTempLen = 0;
	ACE_OS::memset(szTemp, 0, sizeof(szTemp));
	sprintf_safe(szTemp, MAX_BUFF_1024, "%lx" PKG_MSG_CODE, u2PostCommandID);
	nTempLen += ACE_OS::strlen(szTemp);
	ACE_OS::memset(szTemp, 0, sizeof(szTemp));
	sprintf_safe(szTemp, MAX_BUFF_1024, "%ld", BodyPacket.m_nDataLen + nTempLen);
	nTempLen += ACE_OS::strlen(szTemp);
	ACE_OS::memset(szTemp, 0, sizeof(szTemp));
	sprintf_safe(szTemp, MAX_BUFF_1024, "%lx" PKG_MSG_CODE "%ld", u2PostCommandID, BodyPacket.m_nDataLen + nTempLen);
	ACE_OS::memcpy(&szTemp[ACE_OS::strlen(szTemp)], BodyPacket.m_pData, BodyPacket.m_nDataLen);

	WebSocketEncrypt(szTemp, (uint32)ACE_OS::strlen(szTemp), szResponseData, u4ResponseDataLen);
	...
	if(NULL != m_pServerObject->GetConnectManager())
	{
		...
		const char * pszResponseData = szResponseData;
		m_pServerObject->GetConnectManager()->PostMessage(pMessage->GetMessageBase()->m_u4ConnectID, pszResponseData,
			u4ResponseDataLen, SENDMESSAGE_JAMPNOMAL, u2PostCommandID, PACKET_SEND_IMMEDIATLY, PACKET_IS_SELF_RECYC);
		////发送全部数据
		//m_pServerObject->GetConnectManager()->PostMessage(pMessage->GetMessageBase()->m_u4ConnectID, pResponsesPacket,
		//	SENDMESSAGE_JAMPNOMAL, u2PostCommandID, PACKET_SEND_IMMEDIATLY, PACKET_IS_SELF_RECYC);
	}
